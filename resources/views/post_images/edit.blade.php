...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold mb-6">Sửa bài viết nổi bật</h1>
    ...
    <form action="{{ route('post-images.update', $postImage) }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded-lg p-6">
        @csrf
        @method('PUT')

        <div class="mb-4">
            <label for="post_id" class="block text-gray-700 text-sm font-bold mb-2">Post:</label>
            <select name="post_id" id="post_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                ...
                @foreach($posts as $post)
                    <option value="{{ $post->id }}" {{ $post->id == $postImage->post_id ? 'selected' : '' }}>{{ $post->title }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-4">
            ...
            <img src="{{ asset('storage/' . $postImage->image) }}" alt="Current Image" class="w-32 h-32 object-cover mb-2">
        </div>
